...
class Pyramid_Model5():
    def __init__(self, s, w, l2_reg, model_type, embeddings, num_features, d0=300, di=150, d_att1 = 8, d_att2 = 16, w_att1 = 6, w_att2 = 4, num_classes=3, num_layers=2, corr_w = 0.1):
        ...
        with tf.variable_scope("shared-output-layer"):
            self.shared_estimation = tf.contrib.layers.fully_connected(
        ...
        #adversarial loss
        log_soft = tf.nn.log_softmax(self.shared_estimation)  # batch_size * num_corpus
        soft = tf.nn.softmax(self.shared_estimation)
        H_mid = tf.reduce_mean(tf.multiply(soft, log_soft), axis=0)  # [num_corpus]
        self.shared_cost = tf.reduce_sum(H_mid)
        ...
        # 0.001 is the lambda
        self.src_cost = self.src_pred_cost + tf.reduce_sum(l2_reg_lambda*self.corr_trace*self.corr_w) + adv_weight*self.shared_cost

        # 0.001 is the lambda
        self.tgt_cost = self.tgt_pred_cost + tf.reduce_sum(l2_reg_lambda*self.corr_trace*self.corr_w) + adv_weight*self.shared_cost
